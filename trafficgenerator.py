...
def replayTraffic(device,overlap):
    tcpreplaystring = "sudo tcpreplay -p 100000 --loop=1000 --limit 791615 --preload-pcap -i netmap:%s attack_pcaps/ddos.pcap" % device
    tcpreplaystring2 = "sudo tcpreplay -p 100000 --loop=1000 --limit 791615 --preload-pcap -i netmap:%s normal_pcaps/normal%s.pcap" % (device,overlap)
